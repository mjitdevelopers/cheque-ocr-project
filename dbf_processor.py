...
# -*- coding: utf-8 -*-
"""
AI-Powered DBF Issuer Name Processor - FULL UPDATED VERSION
Supports: Cheque images (.tiff, .tif, .jpg, .png) + Existing DBF
Outputs: CSV + DBF with AI-corrected issuer names
"""
...
class DBFIssuerProcessor:
    ...
    def __init__(self, template_dbf_path, output_dbf_path=None):
        self.template_path = Path(template_dbf_path)
        if not self.template_path.exists():
            raise FileNotFoundError(f"Template DBF not found: {template_dbf_path}")

        self.output_path = Path(output_dbf_path) if output_dbf_path else self.template_path.parent / f"updated_{self.template_path.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.dbf"
        ...
        logger.info(f"📁 Output DBF: {self.output_path}")
    ...
    # rest of process() function remains same
    def process(self, image_dir, filename_field='IMAGE_FILE', issuer_field='DRAWER_NAME'):
        ...
        if not any(f[0].upper() == issuer_field.upper() for f in self.field_specs):
            self.field_specs.append((issuer_field, 'C', 100, 0))
            logger.info(f"➕ Adding new field: {issuer_field}")
        ...
        try:
            ...
            temp_csv = self.output_path.with_suffix('.temp.csv')
            df.to_csv(temp_csv, index=False, encoding='utf-8')
            dbf = Dbf5(str(temp_csv))
            dbf.to_dbf(str(self.output_path))
            if temp_csv.exists():
                temp_csv.unlink()
            logger.info(f"✅ DBF file created: {self.output_path}")
        except Exception as e:
            logger.warning(f"⚠️ Could not create DBF: {e}")
            logger.info(f"📌 Use the CSV file instead: {csv_path}")
        ...
        logger.info("="*60)
        return {'total': len(records), 'matched': matched_count, 'extracted': extracted_count, 'output': str(self.output_path)}


def main():
    import argparse
    parser = argparse.ArgumentParser(description='AI DBF Issuer Name Processor')
    ...
    parser.add_argument('--output', '-o', help='Output DBF file')
    ...
    processor.process(image_dir=args.images, filename_field=args.filename_field, issuer_field=args.issuer_field)
