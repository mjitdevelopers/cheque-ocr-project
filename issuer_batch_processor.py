...
class IssuerBatchProcessor:
    ...
    def is_valid_drawer(self, text):
      if not text:
        return False
    # Remove known junk or placeholders
      invalid_words = ["PLEASE SIGN", "PROPRIETOR", "CURRENT", "PARTNERS", "XXX", "SIGN"]
      for w in invalid_words:
        if w in text.upper():
            return False
    # Skip bank names
      bank_words = ["BANK", "STATE BANK", "HDFC", "HDFC BANK LTD.","ICICI", "SBI", "AXIS", "YES", "IDFC", "KOTAK", "INDUSIND", "PNB"]
      for w in bank_words:
        if w in text.upper():
            return False
      return True
